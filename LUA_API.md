...
2. **`フォルダ名.lua`**（例: `/games/Run!Yamame/Run!Yamame.lua`）
...
|----------------|-------------------------------|
| `/games/Run!Yamame/Run!Yamame.lua` | `Run!Yamame/title.bin` → `preview.bin` → **`Run!Yamame.bin`** |
| `/games/puzzle.lua` | `/games/title.bin` → `/games/preview.bin` → **`/games/puzzle.bin`** |

プレビュー用 `.bin` の変換例（100×100）:

```bash
python tool/image_to_rgb565_bin.py artwork.png --resize 100x100 -o "games/Run!Yamame/Run!Yamame.bin"
```

64×64 など小さい `.bin` も表示できます（中央配置）。`tool/dino.bin` は 64×64（8,192 バイト）の旧サンプルです。
...
/games/
├── Run!Yamame/
│   ├── Run!Yamame.lua    # 起動（フォルダ名と同名）
│   ├── Run!Yamame.bin    # メニュー右のプレビュー（任意）
│   └── cave.bin          # ゲーム内アセット
...
2. **高速パス（既定）**: `game_draw()` を **1 回だけ**実行して描画コマンドを録画（約 12KB）→ dirty なバンドだけ C 側で再生し LCD へ DMA  
3. **フォールバック**: 録画失敗時や `layers` モード時は、従来どおりバンドごとに `game_draw()`（例: 240÷20＝12 回）  
4. （ビルド時 `GAME_MACHINE_DEBUG=ON` なら）画面右上に FPS / RAM をオーバーレイ表示（250ms 更新）  
5. エンコーダ音量のポーリング  

`draw_vn_stream` / `draw_bg_stream` の SD ファイルは **フレーム末で閉じず**、パスが変わらない限り開きっぱなし（ゲーム終了時に解放）。  
DMA 転送はバウンスバッファを 2 分割し、バイトスワップ準備と SPI 転送を重ねる。
...
毎フレーム `machine.clear()` してから全体を描き直す方式（`Run!Yamame.lua` / `stg.lua`）は **`direct` モード**向け。
...
| `machine.fill_rect_alpha(x, y, w, h, color, alpha)` | 上記 + `alpha` … 0〜255（0=透明、255=不透明） | なし | 現在バンドの既存ピクセルに RGB565 を α 合成する。背景暗幕などに使う。 |
...
| `machine.draw_image_affine(id, a, b, c, d, e, f [, key])` | 2×3 アフィン行列。ソース座標 `(u,v)` → 画面 `(a*u+b*v+c, d*u+e*v+f)`。<br>`key` 省略=不透過、`true`=マゼンタ透過、数値=透過色 RGB565 | なし | 拡大・回転・反転など。ニアレストサンプリング。帯にクリップ。<br>**例外**: 回転・せん断なしの正の整数倍スケール（例: `2,0,0,0,2,0`）は描画録画対応の高速パス。 |
| `machine.draw_image_xform(id, opts)` | `opts` テーブル（下記）。便利な拡大・回転・反転 API | なし | 内部でアフィン行列を組み立てて `draw_image_affine` 相当を実行。 |
...
| `machine.draw_sprite_affine(...)` | `draw_image_affine` と同じ | なし | 同上。 |
| `machine.draw_sprite_xform(...)` | `draw_image_xform` と同じ | なし | 同上。 |
...
**`draw_image_xform` の opts テーブル**

| キー | 省略時 | 意味 |
|------|--------|------|
| `x`, `y` | 0 | 画面上のアンカー位置（ピボットの着地点） |
| `ox`, `oy` | 画像中心 | ソース側ピボット（画像ピクセル座標） |
| `scale` | 1 | 縦横同じ倍率（`scale_x`/`scale_y` より先に適用） |
| `scale_x`, `scale_y` | 1 | 軸別倍率（負値でも可。`flip_*` と併用可） |
| `angle` | 0 | 回転角（**度**、数学座標で反時計回り） |
| `flip_x`, `flip_y` | false | true でその軸を反転（倍率に -1 を掛ける） |
| `sx`, `sy`, `sw`, `sh` | 全体 | ソース矩形 |
| `keyed` | false | true で透過キー有効 |
| `key` | `0xF81F` | 透過色。数値を指定すると `keyed` も有効化 |

```lua
-- 2 倍・45 度回転・マゼンタ透過
machine.draw_image_xform(player_id, {
  x = 160, y = 120,
  scale = 2,
  angle = 45,
  keyed = true,
...
machine.draw_image_affine(player_id, 2, 0, 10, 0, 2, 20, true)
```
...
| 関数 | 引数 | 戻り値 | 意味 |
|------|------|--------|------|
| `machine.load_return(path)` | SD 上の `.lua` パス | 成功: `return` 値 1 個<br>失敗: `nil`, errmsg | 別ファイルを読み込み **1 回だけ** 実行して戻り値を得る（`scenario.lua` / `assets.lua` 向け）。ファイルサイズ上限は起動スクリプトと同じ **96KB**。 |
