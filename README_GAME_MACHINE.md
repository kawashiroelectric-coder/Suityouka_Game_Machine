...
ルートまたは任意フォルダに **`.lua` とアセット**を配置します。メニュー運用ではリポジトリの [`games/`](games/) を SD の **`/games/`** にコピーするのが一般的です。
...
│   ├── Shogi.lua
│   ├── fonts/game_font.bin
│   ├── img/          … Momiji_*.bin, Koma.bin, Shogi_BGs.bin
│   └── sound/piece_drop.wav
├── Run!Yamame/
│   ├── Run!Yamame.lua
│   └── *.bin         … run / slide / cave / bat 等
├── visual_novel/
│   ├── visual_novel.lua
│   ├── scenario.lua
│   ├── fonts/
...
サンプル詳細: [games/README.md](games/README.md)
...
| `games/` | Lua サンプルゲーム（[games/README.md](games/README.md)） |
...
- **VN ストリーム合成**: 背景 + 立ち絵（最大 2 枚）は `machine.draw_vn_stream` で SD から帯ごと合成（`games/visual_novel/` 参照）
