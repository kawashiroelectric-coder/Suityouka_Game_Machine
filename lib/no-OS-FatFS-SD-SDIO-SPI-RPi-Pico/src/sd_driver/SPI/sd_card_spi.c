 ...
 */
/*
 * Modified for Suityouka Game Machine (Kawashiro Electric, 2026).
 * See MODIFICATIONS.md in the library root and THIRD_PARTY_NOTICES.md in the project root.
 */
/*
...
/** SDHC/SDXC はブロックアドレス、SDSC (V1/V2) はバイトアドレス */
static bool sd_card_is_high_capacity(const sd_card_t *sd_card_p) {
    return sd_card_p->state.card_type == SDCARD_V2HC;
}

static uint32_t sd_sector_to_wire_addr(const sd_card_t *sd_card_p, uint32_t sector) {
    if (sd_card_is_high_capacity(sd_card_p)) {
        return sector;
    }
    return sector * (uint32_t)sd_block_size;
}
...
static block_dev_err_t in_sd_read_blocks(sd_card_t *sd_card_p, uint8_t *buffer,
    ...
    const uint32_t wire_addr = sd_sector_to_wire_addr(sd_card_p, data_address);
    if (num_rd_blks == 1)
        status = sd_cmd(sd_card_p, CMD17_READ_SINGLE_BLOCK, wire_addr, false, 0);
    else
        status = sd_cmd(sd_card_p, CMD18_READ_MULTIPLE_BLOCK, wire_addr, false, 0);
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) return status;
                                          ...
                                          uint32_t * const num_wrt_blks_p)
{
    ...
    status = sd_cmd(sd_card_p, CMD25_WRITE_MULTIPLE_BLOCK,
                    sd_sector_to_wire_addr(sd_card_p, *data_address_p), false, 0);
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) return status;
                                          ...
                                          uint32_t const address)
{
    ...
    status = sd_cmd(sd_card_p, CMD24_WRITE_BLOCK, sd_sector_to_wire_addr(sd_card_p, address),
                    false, 0);
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) return status;
...
static block_dev_err_t sd_init_medium(sd_card_t *sd_card_p) {
    ...
    if (SD_BLOCK_DEVICE_ERROR_NONE != status && SD_BLOCK_DEVICE_ERROR_UNSUPPORTED != status) {
        EMSG_PRINTF("CMD8 failed: status=%ld card_type=%d\n", (long)status,
                    (int)sd_card_p->state.card_type);
        return status;
    }
    if (SD_BLOCK_DEVICE_ERROR_UNSUPPORTED == status) {
        DBG_PRINTF("CMD8 unsupported (legacy/MMC?); continuing as v1 path\n");
    }
    ...
    /* SPI init: OCR is often invalid (0) before ACMD41. CMD8 already verified
     * 2.7-3.6V via VHS. Read OCR after ACMD41 for CCS (see jxglib / SD spec order). */
    ...
    if (SDCARD_V2 == sd_card_p->state.card_type) {
        ...
        if (SD_BLOCK_DEVICE_ERROR_NONE ==
            ...
            if (response & OCR_HCS_CCS) {
                ...
                /* SPI: OCR may read as 0 after ACMD41; CSD structure v2.0 => SDHC/SDXC. */
                if (sd_cmd(sd_card_p, CMD9_SEND_CSD, 0x0, false, 0) == 0
                    && read_bytes(sd_card_p, sd_card_p->state.CSD, 16) == 0
                    && ext_bits16(sd_card_p->state.CSD, 127, 126) == 1) {
                    sd_card_p->state.card_type = SDCARD_V2HC;
                    DBG_PRINTF(
                        "Card Initialized: High Capacity Card (CSD v2, OCR unreadable)\n");
                } else {
    ...
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) {
        EMSG_PRINTF("ACMD42 failed: status=%ld\n", (long)status);
    }
...
DSTATUS sd_card_spi_init(sd_card_t *sd_card_p) {
    ...
    if (SDCARD_V2HC == sd_card_p->state.card_type) {
        DBG_PRINTF("SD card type: SDHC/SDXC (block addressing)\n");
    } else if (SDCARD_V2 == sd_card_p->state.card_type || SDCARD_V1 == sd_card_p->state.card_type) {
        DBG_PRINTF("SD card type: SDSC (byte addressing, max ~2 GiB)\n");
    } else {
        EMSG_PRINTF("Unknown or unsupported SD card type\n");
        sd_release(sd_card_p);
        return sd_card_p->state.m_Status;
    }
