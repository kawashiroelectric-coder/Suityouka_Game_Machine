...
/* return Capacity in sectors (512-byte units; SDXC max ~2 TiB) */
static inline uint32_t CSD_sectors(CSD_t csd) /*  const  */ {
    ...
    if (ver == 0) {
        ...
        /* memory capacity = (C_SIZE+1) * 512K byte; sector count = (C_SIZE+1) * 1024 */
        const uint64_t sectors = (uint64_t)(c_size + 1) * 1024ULL;
        if (sectors == 0) {
            return 0;
        }
        /* SDXC max is 2^32 sectors; uint32_t LBA range covers 0 .. sectors-1 */
