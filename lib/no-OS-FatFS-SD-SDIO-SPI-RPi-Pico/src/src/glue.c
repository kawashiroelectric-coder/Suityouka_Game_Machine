...
/** FatFs LBA_t (64-bit when FF_LBA64) → SD ドライバ uint32_t セクタ番号 */
static int lba_to_sector32(LBA_t sector, uint32_t* out) {
...
DRESULT disk_read(BYTE pdrv,  /* Physical drive number to identify the drive */
    ...
    if (!sd_card_p) return RES_PARERR;
    uint32_t sector32;
    if (lba_to_sector32(sector, &sector32) != RES_OK) {
        return RES_PARERR;
    }
    int rc = sd_card_p->read_blocks(sd_card_p, buff, sector32, count);
...
DRESULT disk_write(BYTE pdrv, /* Physical drive number to identify the drive */
    ...
    int rc = sd_card_p->write_blocks(sd_card_p, buff, sector32, count);
    return sdrc2dresult(rc);
}
...
DRESULT disk_ioctl(BYTE pdrv, /* Physical drive number (0..) */
    ...
    switch (cmd) {
        case GET_SECTOR_COUNT: {  // Retrieves number of available sectors, the
            ...
            const uint32_t n32 = sd_card_p->get_num_sectors(sd_card_p);
            if (!n32) return RES_ERROR;
            *(LBA_t *)buff = (LBA_t)n32;
            return RES_OK;
        }
